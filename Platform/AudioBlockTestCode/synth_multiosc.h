...
{
public:
	AudioSynthMultiOsc() : AudioStream(9, inputQueueArray), magnitude(16384){
		staticAmp[0] = 0;
		staticAmp[1] = 0;
		staticAmp[2] = 0;
		staticAmp[3] = 0;
...
//		magnitude = n * 65536.0;
//	}
	void setAmp(uint8_t osc, uint8_t level) {
		if(osc < 4) staticAmp[osc] = level;
	}
	virtual void update(void);
private:
	uint32_t phase_accumulator[4];
	uint32_t phase_increment;
	int32_t magnitude;
public:
	uint32_t staticAmp[4];
};
